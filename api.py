...
def extract_query_details(query):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        Consider that:
        - A name could be a single word (e.g., 'Luna') or multiple words (e.g., 'John Doe').
        - An ID is typically a number (e.g., '12345') or alphanumeric code.
        - The query might be phrased as 'I want information about [name/ID]' or simply '[name/ID]'.
...
def get_conversational_chain(tools, ques):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a helpful assistant. Use the retrieved documents and past conversation to understand user intent. 
...
def process_user_input(user_question):
    ...
    if pending_query is not None:
        # Check if the user's response indicates they don't know the requested information
        if is_lack_of_knowledge_response(user_question):
            response = "I understand. I can't provide this information without a specific customer name or ID. You can ask me any other questions, and I'll be happy to help with those instead."
            chat_history.append(HumanMessage(content=user_question))
            chat_history.append(AIMessage(content=response))
            pending_query = None
            return {"status": "success", "message": response, "pending_cleared": True}
        
        # Combine pending query with new input
